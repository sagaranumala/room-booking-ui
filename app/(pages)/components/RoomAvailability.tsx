...
import { getAllRoomsAvailabilitySlots } from "@/services/api";
import { RoomAvailabilitySlots } from "@/app/(pages)/types";
import { useRouter } from "next/navigation";

export default function RoomAvailability() {
  const router = useRouter();
  const [date, setDate] = useState<string>(format(new Date(), "yyyy-MM-dd"));
  const [rooms, setRooms] = useState<RoomAvailabilitySlots[]>([]);

  const fetchAvailability = async (selectedDate: string) => {
    try {
      const res = await getAllRoomsAvailabilitySlots(selectedDate);
        ...
        <div key={roomData.room.id} className="border rounded-lg shadow p-4 bg-white">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-2">
            {roomData.availableSlots.map((slot, index) => {
              const isNext = slot.start === roomData.nextAvailableSlot;
              const isLast = slot.start === roomData.lastAvailableSlot;
              return (
                <div
                  key={index}
          ...
          <div className="mt-4 text-sm text-gray-600">
            Total Slots: {roomData.totalAvailableSlots} | Total Bookings: {roomData.totalBookings} | Available: {roomData.isAvailable ? "Yes" : "No"}
          </div>
        </div>
      ))}
    </div>
